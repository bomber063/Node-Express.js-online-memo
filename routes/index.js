...
var data;
if(req.user){//这里因为在auth.js的注销中用了req.logout();，所以会清除req.user，把它设置为null。当然也可以写req.session.user,因为在auth.js的注销中也用了req.session.destroy()，会清除掉req.session，注意单独的req.logout()能清除掉req.user,但是不能清除掉req.session
  data = {
    isLogin: true,
